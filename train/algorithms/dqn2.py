...
from train.utils.agentsAttention import DQNAttentionAgent
import numpy as np
...
class DQN(object):
    ...
    def __init__(self, agent_init_params, alg_types,
        ...
        self.agents = [DQNAttentionAgent(lr=lr, discrete_action=discrete_action,
    ...
    def update(self, sample, agent_i, parallel=False, logger=None):
        ...
        actual_values=curr_agent.policy(curr_obs_x1,curr_obs_x2).gather(1,curr_acs_index)
        target_values=curr_rews.view(-1, 1)+self.gamma*(1 - dones[agent_i].view(-1, 1))*curr_agent.target_policy(curr_next_obs_x1,curr_next_obs_x2).max(1)[0].unsqueeze(1).detach()
    ...
    @classmethod
    def init_from_env(cls, agent_num=3, agent_alg="DQN", num_in_pol=360, attention_dim=2, num_out_pol=4, discrete_action=True,
                      gamma=0.95, tau=0.01, lr=0.01, hidden_dim=32):
        """
